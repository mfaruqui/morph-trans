...
int main(int argc, char** argv) {
  ...
  for (unsigned iter = 0; iter < num_iter; ++iter) {
    ...
    for (string& line : train_data) {
      ...
      loss[morph_id] += nn.Train(morph_id, iter + 1, input_ids, target_ids,
                                 &lm, &optimizer[morph_id]);
      cerr << ++line_id << "\r";
    }

    // If its the first iteration do not evaluate on dev data yet.
    // The LM weights are not trained in the first iteration.
    if (iter == 0) {
      continue;
    }
