...
NoEnc::NoEnc(const unsigned& char_length, const unsigned& hidden_length,
...
void NoEnc::InitParams(vector<Model*>* m) {
...
void NoEnc::AddParamsToCG(const unsigned& morph_id, ComputationGraph* cg) {
...
/*void NoEnc::RunFwdBwd(const unsigned& morph_id,
...
void NoEnc::TransformEncodedInput(Expression* encoded_input) const {
  *encoded_input = affine_transform({transform_encoded_bias,
                                     transform_encoded, *encoded_input});
}*/

void NoEnc::ProjectToOutput(const Expression& hidden, Expression* out) const {
  *out = affine_transform({hidden_to_output_bias, hidden_to_output, hidden});
}

Expression NoEnc::ComputeLoss(const vector<Expression>& hidden_units,
...
float NoEnc::Train(const unsigned& morph_id, const vector<unsigned>& inputs,
    ...
    if (i < outputs.size() - 1) { 
      ...
      if (i < inputs.size() - 1) {
        input_vecs_for_dec.push_back(concatenate(
            {lookup(cg, char_vecs[morph_id], outputs[i]),
...
EnsembleDecode(const unsigned& morph_id, unordered_map<string, unsigned>& char_to_id,
  ...
  /*auto temp_model = *(*ensmb_model)[0];
  for (unsigned char_id = 0; char_id < temp_model.vocab_len; ++char_id) {
     for (unsigned index = 0; index < temp_model.char_len; ++index) {
       cout << temp_model.char_vecs[0]->values[char_id].v[index] << " ";
     }
     cout << endl;
  }
  exit(0);*/
  ...
  while (pred_target_ids->size() < MAX_PRED_LEN) {
    ...
    for (unsigned ensmb_id = 0; ensmb_id < ensmb; ++ensmb_id) {
      ...
      input = concatenate({//encoded_word_vecs[ensmb_id],
                           prev_output_vec,
                           input_char_vec});
...
EnsembleBeamDecode(const unsigned& morph_id, const unsigned& beam_size, 
  ...
  for (unsigned ensmb_id = 0; ensmb_id < ensmb; ++ensmb_id) {
    ...
    Expression prev_output_vec = lookup(cg, model.char_vecs[morph_id],
                                        char_to_id[BOW]);
    Expression input = concatenate({//encoded_word_vecs[ensmb_id],
                                    prev_output_vec,
...
void Serialize(string& filename, NoEnc& model, vector<Model*>* cnn_models) {
...
void Read(string& filename, NoEnc* model, vector<Model*>* cnn_models) {
