...
int main(int argc, char** argv) {
  ...
  for (unsigned iter = 0; iter < num_iter; ++iter) {
    ...
    for (string& line : test_data) {
      ...
      unsigned morph_id = morph_to_id[items[2]];
      EnsembleDecode(char_to_id, input_ids, &pred_target_ids, &nn[morph_id]);
